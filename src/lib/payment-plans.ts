...
function clampToToday(dueDate: string): string {
  const now = today();
  return dueDate < now ? now : dueDate;
}
...
export function layoutSlots(
  ...
  if (planType === "semester") {
    return layoutSemesterSlots(remaining, year.sem1DueOn, year.sem2DueOn);
  }
  return layoutMonthlySlots(
    remaining,
    monthlyAmount ?? 0,
    year.monthlyDueDay,
  );
